...
// ---------------------------------------------------------------------------
// 
// (C) 2010-2023, Association pour le Bien des Aveugles
...
// -----------------------------------------------------------------------------
import { Config, LogTo, Loggers } from "./config.js";
...
(() => {
    ...
    class CorsProxy {
         ...
         */
        proxied_url(url) {
            return this.#proxy_url ? this.#proxy_url + url : url;
        }
    }
    ...
    class Utils {
        ...
        static get_base_url(url) {
            return url.replace(url.split('/').reverse()[0], '');
        }
        ...
        static truncate(text, n = 35) {
            ...
            return (data.length > n) ? data.substr(0, n - 1) + ELLIPSIS : data;
        }
        ...
        static fetch_xml = async (url) => {
            ...
            const _logger = Loggers[CONTEXT] || LogTo.NO_LOGGER;
    ...
    class Bookmark {
         ...
         * @param {Number} smil_index The referenced SMIL index
         * @param {Number} clip_index The referenced CLIP Index
         */
        constructor(id, title, smil_index, clip_index) {
            const CONTEXT = 'Bookmark.constructor';
            this._logger = Loggers[this.constructor.name] || LogTo.NO_LOGGER;
        ...
        /**
         * Return a bookmark as a `JSON` dictionnary.
         * 
        ...
        /**
         * Return an `HTML` list item representing this bookmark.
         * 
         * 
         * @returns An <li>...</li> element
         */
        as_html_list_item() {
    ...
    */
    class GuiElement {
        ...
        constructor(id, title = null, content = null) {
            ...
            this.#logger = Loggers[this.constructor.name] || LogTo.NO_LOGGER;
            this.#logger(CONTEXT, `Creating [${id}], title=${title}, content=${content}`);
            this.id = id;
            this.title = title;
            this.content = content;
            this.#jqo = $(`#${this.id}`); // Private jQuery object
            this.#element = document.getElementById(this.id); // JS element

            if (!this.#element) {
                this.#logger(CONTEXT, `Element [${id}] not found in the DOM !`)
                return;
            }
            ...
            if (this.title) {
                ...
                this.#element.ariaLabel = this.title;
            }
        ...
        progress(percent) {
            ...
            this.#jqo.progress({ percent: percent });
        }
    ...
    class DaisyPlayer {
        ...
        constructor(ncc, autoplay) {
            const CONTEXT = 'DaisyPlayer.constructor';
            // Logging
            this._logger = Loggers[CONTEXT] || LogTo.NO_LOGGER;
            this._logger(CONTEXT, 'DaisyPlayer constructor called...');

            // Initialize the class members
            this.ncc = ncc;
            this.autoplay = autoplay;
            this.valid_ncc = ncc && ncc.has_title();
            ...
            this._logger(CONTEXT, 'Attaching media event handlers...');
            this.events.items().forEach(item => {
                this.player.addEventListener(item.name, item => this.media_event_handler(item));
            });
            this._logger(CONTEXT, 'Attaching media event handlers done.');
            ...
            this._logger(CONTEXT, 'DaisyPlayer constructor finished.');
        }
        ...
        navigate_to(section, index = null) {
            ...
            const _logger = Loggers[CONTEXT] || LogTo.NO_LOGGER;
            _logger(CONTEXT, `BookSection: ${section}, Navigation level: ${this.navigation_level}, Index: ${index}`);
            var smil_reference = null;
            switch (section) {
                case BookSections.SPECIFIC:
                    smil_reference = this.ncc.smil_section_references[index];
                    _logger(CONTEXT, 'Found specific:', smil_reference, smil_reference.id === index);
                ...
                case BookSections.FIRST:
                    for (let i = 0; i <= this.ncc.max_section_index(); i++) {
                        ...
                        if (smil_reference.level === this.navigation_level) {
                            _logger(CONTEXT, 'Found first:', smil_reference);
                ...
                case BookSections.NEXT:
                    for (let i = this.section_index + 1; i <= this.ncc.max_section_index(); i++) {
                        smil_reference = this.ncc.smil_section_references[i];
                        if (smil_reference.level === this.navigation_level) {
                            _logger(CONTEXT, 'Found next:', smil_reference);
                            this.section_index = i;
                            break;
                        }
                    }
                ...
                case BookSections.PREVIOUS:
                    for (let i = this.section_index - 1; i >= 0; i--) {
                        smil_reference = this.ncc.smil_section_references[i];
                        if (smil_reference.level === this.navigation_level) {
                            _logger(CONTEXT, 'Found prev:', smil_reference);
                            this.section_index = i;
                            break;
                        }
                    }
                ...
                case BookSections.LAST:
                    for (let i = this.ncc.max_section_index(); i >= 0; i--) {
                        smil_reference = this.ncc.smil_section_references[i];
                        if (smil_reference.level === this.navigation_level) {
                            _logger(CONTEXT, 'Found last:', smil_reference);
                            this.section_index = i;
                            break;
                        }
                    }
        ...
        async play_section(section) {
            const CONTEXT = 'DaisyPlayer.play_section';
            const _logger = Loggers[CONTEXT] || LogTo.NO_LOGGER;
            _logger(CONTEXT, `BookSection: ${section}, Navigation level: ${this.navigation_level}`);
            switch (section) {
                case BookSections.NEXT:
         ...
         */
        on_click(gui_event) {
            const CONTEXT = 'DaisyPlayer.on_click';
            const _logger = Loggers[CONTEXT] || LogTo.NO_LOGGER;
            _logger(`${CONTEXT} GUI event: ${gui_event}`);
            // Handle GUI events    
            switch (gui_event) {
        ...
        media_event_handler(event) {
            const CONTEXT = 'DaisPlayer.media_event_handler';
            const _logger = Loggers[CONTEXT] || LogTo.NO_LOGGER;
            _logger(CONTEXT, 'Event: ' + event.type);
    ...
    /**
     * This class represents the meta data information found in the various files of a Daisy project.
     */
    class MetaData {
        /**
    ...
    class Document {
        ...
        constructor(url, content) {
            ...
            this.#content = content; // content.getElementsByTagName('*');
            this.#is_valid = (this.#url !== null) && (this.#content !== null);
        }
        ...
        get_element(tag_name) {
            return this.#content.querySelector(tag_name);
        }

        get_elements(tag_names) {
            return this.#content.querySelectorAll(tag_names);
        }

        get_body() {
            return this.#content.querySelectorAll('body')[0];
        }

        get_fragment(anchor) {
            return this.#content.querySelectorAll(anchor)[0];
        }
    }
    ...
    class FullTextDocument {
        ...
        get_base_url() {
            return this.#document.get_base_url();
        }

        get_url() {
            return this.#document.get_url();
        }

        get_body() {
            return this.#document.get_body();
        }

        get_fragment_text(anchor) {
            const fragment = this.#document.get_fragment(anchor);
            return fragment ? fragment.textContent.trim() : '';
        }

        process() {
            if (!this.#document.is_valid())
                return;

            // Remove unneeded attributes for a raw display    
            this.#document.get_elements('*').forEach((item) => {
            ...
            this.#document.get_elements('a, em, strong, q, i, big').forEach(item => {
                item.parentElement.append(item.firstChild); // Move the first chil up
                item.remove();
            });

            // Update the image tags to point to the right source
            this.#document.get_elements('img').forEach((item) => {
                const ATTR_NAME = 'src';
                item.setAttribute(ATTR_NAME, this.#document.get_base_url() + item.getAttribute(ATTR_NAME));
            });

            // Page numbering
            this.#document.get_elements('.page-normal').forEach((item) => {
    ...
    class FullTextDocumentContainer {
        ...
        add(document) {
            const exists = this.#documents.find(item => item.get_url() === document.get_url()) !== undefined;
            if (exists)
                return;
        ...
        document_exists(url) {
            return this.#documents.find(item => item.get_url() === url) ? true : false;
        }

        find_document_by_url(url) {
            return this.#documents.find(item => item.get_url() === url);
        }
    }

    class Clip {
        #CONTEXT = 'Clip';
        #logger = Loggers[this.#CONTEXT] || LogTo.NO_LOGGER;
        constructor(index, media_url, text_anchor, start_sec, end_sec, text) {
            this.#logger(this.#CONTEXT, `Constructor | index: ${index}, start: ${start_sec}, end: ${end_sec}, media: ${media_url}, anchor: ${text_anchor}, text: ${text}`);
            this.index = index;
    ...
    class Smil {
        ...
        async process() {
            ...
            if (this.parent.fulltext_document_container.document_exists(this.fulltext_url)) {
        ...
        process_clips() {
            ...
            $(this.pars).each((index, item) => {
                ...
                const fulltext_document = this.parent.fulltext_document_container.find_document_by_url(this.fulltext_url);
                if (!fulltext_document) {
                    return;
                }

                const fragment_text = fulltext_document.get_fragment_text('#' + text_anchor);
                ...
                const clip = new Clip(index, clip_url, text_anchor, clip_start, clip_end, fragment_text);
    ...
    class Ncc {
        ...
        constructor(document) {
            ...
            this.#document = document;
            this.#base_url = this.#document.get_base_url();

            // We have a good NCC if there is a Dublin Core title (dc:title)
            const dc_title = this.#document.get_element('meta[name="dc:title"]');
            ...
            const source_title = this.#document.get_element('h1.title');
            this.title = source_title ? source_title.textContent : 'NO_TITLE';

            // Build an UID (used for bookmarking)
            const dc_identifier = this.#document.get_element('meta[name="dc:identifier"]');
            this.uid = Utils.generate_uid(this.#document.get_url(), (dc_identifier ? dc_identifier.content : ''));

            // Get the meta tags
            this.#document.get_elements('meta').forEach(item => {
                if (item.name)
                    this.meta_tags.push(new MetaData(item.name, item.content, item.scheme));
            });

            // Process the <h1> to <h6> and <span> tags
            let index = 0, section_index = 0;
            this.#document.get_elements('h1, h2, h3, h4, h5, h6, span').forEach(item => {
                ...
                if (tag_name === 'span') {
                    ...
                    level = parseInt(tag_name[1], 10); // 1 to 6
        ...
        document_exists() {
            return this.#document.get_content() !== null;
        }
    ...
    class Factories {
        static create_document = (async url => {
            const source = await Utils.fetch_xml(url);
            return new Document(url, source);
        });

        static async create_ncc(url) {
            const source = await Utils.fetch_xml(url);
            const document = await Factories.create_document(url);
            return new Ncc(document)
        }
        ...
        static async create_full_text_document(url) {
            const document = await this.create_document(url);
            return new FullTextDocument(document);
        }
    }
    ...
    */
    ...
    async function main() {
        ...
        // Check if we have an NCC
        if (!ncc.document_exists()) {
            Utils.throw_error_body(I18N.Tr('INVALID_NCC').replace(I18N.parameter_1, ncc.document_url()));
            return;
        }

        // Check if it is valid (title)
        if (!ncc.has_title()) {
            Utils.throw_error_body(I18N.Tr('ERROR_NCC_TITLE'));
            return;
        }
